...
import java.time.LocalDate;
    ...
    public class TaskManager implements Serializable {
        ...
        private FileHandler todoFile = new FileHandler();
        ...
        public TaskManager() {
            tasks = todoFile.taskLoadToArray();
        }
            ...
            try {
                ...
                if ((edit_task_due_date.compareTo(current_date)) < 0) {
                    ...
                    if (edit_projectname.isEmpty() || edit_status.isEmpty() || new_dueDate.isEmpty() || edit_taskname.isEmpty()) {
                        ...
                        tasks.get(task_index).setDueDate(edit_task_due_date);
                        tasks.get(task_index).setTaskStatus(edit_status);
                        tasks.get(task_index).setProjectName(edit_projectname);
                        System.out.println();
                        System.out.println();
                        System.out.println("Current Task " + (task_index + 1) + " updated with new values: " + tasks.get(task_index).getTaskTitle() + " " + tasks.get(task_index).getDueDate() + "   " + tasks.get(task_index).getTaskStatus() + "   " + tasks.get(task_index).getProjectName());
                    }
                }
            ...
            try {
                ...
                if ((task_due_date.compareTo(current_date)) < 0) {
                    ...
                    if ((new_taskname.isEmpty()) || (new_projectname.isEmpty()) || (new_status.isEmpty()) || (new_dueDate.isEmpty())) {
                        ...
                        tasks.add(new TodoTask(new_taskname, task_due_date, new_status, new_projectname));
                        System.out.println();
        ...
        public void display() {
            ...
            while (index < tasks.size()) {
                System.out.format("%10d %20s %27s %25s %28s", index + 1, tasks.get(index).getTaskTitle(), tasks.get(index).getDueDate(), tasks.get(index).getTaskStatus(), tasks.get(index).getProjectName());
                System.out.println();
                index++;
            }
        }
            ...
            switch (display_option) {
                case "1":
                    ...
                    if (tasks.isEmpty()) {
                        ...
                        System.out.println();
                        tasks.sort((TodoTask task1, TodoTask task2) -> task1.getDueDate().compareTo(task2.getDueDate()));
                        tasks.forEach((sorted_list_date) -> System.out.format("%10s %20s %25s %25s %n", sorted_list_date.getDueDate(), sorted_list_date.getTaskTitle(), sorted_list_date.getTaskStatus(), sorted_list_date.getProjectName()));
                    }
                    break;
                case "2":
                    ...
                    if (tasks.isEmpty()) {
                        ...
                        System.out.println();
                        tasks.sort((TodoTask task1, TodoTask task2) -> task1.getProjectName().compareTo(task2.getProjectName()));
                        tasks.forEach((sorted_list_project) -> System.out.printf("%10s %20s %25s %28s %n", sorted_list_project.getProjectName(), sorted_list_project.getTaskTitle(), sorted_list_project.getTaskStatus(), sorted_list_project.getDueDate()));
                    }
                    break;
                default:
                    System.out.println();
